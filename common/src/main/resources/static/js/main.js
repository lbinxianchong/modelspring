layui.use(['element', 'form', 'layer', 'upload'], function () {
    ...
    var form = layui.form; //加载element模块
    ...
    /* 最大化窗口 */
    $(".mo-screen-full").on("click", function (e) {
        e.preventDefault();
    ...
    var tabs = function (url) {
        ...
        if (url !== undefined && url !== '#' && item.length > 0) {
            ...
            if (bootLay.length === 0) {
                ...
                // var iframeUrl = (window.location.pathname + url);
                var iframeUrl = url;
    ...
    $(".ajax-status").on("click", function (e) {
        e.preventDefault();
        var checked = [];
        var tdcheckbox = $(".mo-table td .mo-checkbox :checkbox:checked");
    ...
    // 展示数据列表-多选框
    var thcheckbox = $(".mo-table th .mo-checkbox :checkbox");
    thcheckbox.on("change", function () {
        var tdcheckbox = $(".mo-table td .mo-checkbox :checkbox");
    ...
    // 检测列表数据是否为空
    var moTable = $(".mo-table tbody");
    if (moTable.length > 0) {
        var children = moTable.children();
        if (children.length === 0) {
            var length = $(".mo-table thead th").length;
            var trNullInfo = "<tr><td class='mo-table-null' colspan='"
                + length + "'>没有找到匹配的记录</td></tr>";
            moTable.append(trNullInfo);
        }
    }
    ...
    var paramSkip = function () {
        var getSearch = "";
        // 搜索框参数
        $('.mo-search-box [name]').each(function (key, val) {
    ...
    /* 展示列表数据搜索 */
    $(document).on("click", ".mo-search-btn", function () {
        paramSkip();
    });
    /* 改变显示页数 */
