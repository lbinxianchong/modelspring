 ...
 * @date 2018/10/24
 */
public class ModelControllerTemplate {
    ...
    private static Document genClazzBody(Generate generate) {
        ...
        String path = FileUtil.templatePath(ModelControllerTemplate.class);

        // 获取jAngel文档对象
        Document document = JavaParseUtil.document(path, expression, generate, TierType.MODEL_CONTROLLER);
    ...
    public static String generate(Generate generate) {
        // 生成文件
        String filePath = GenerateUtil.getJavaFilePath(generate, TierType.MODEL_CONTROLLER);
        try {
