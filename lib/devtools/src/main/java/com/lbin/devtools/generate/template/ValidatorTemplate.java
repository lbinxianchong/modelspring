    ...
    public static Document genClazzBody(Generate generate){
        ...
        generate.getFields().forEach(field -> {
            ...
            if(verifys.size() > 0){
                ...
                // 验证规则集合
                FieldType typeEnum = (FieldType) EnumUtil.enumCode(FieldType.class, field.getType());
                verifys.forEach(verifyCode -> {
                    FieldVerify verifyEnum = (FieldVerify) EnumUtil.enumCode(FieldVerify.class, verifyCode);
